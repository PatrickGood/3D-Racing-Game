...
public class JoglEventListener implements GLEventListener, KeyListener, MouseListener, MouseMotionListener {
	//string new text = Integer.toString(time);
	...
	Racecar user = new Racecar(0,track);
	Racecar car1 = new Racecar(1,track);
	Racecar car2 = new Racecar(2,track);
	Racecar car3 = new Racecar(3,track);
	
	float car_size = 20.0f;
	int camera = 3;
	boolean camera_switch = true;
   ...
   // float materialAmbient2[] = {0.0f, 1.0f, 0.0f};
	...
	public void display( GLAutoDrawable gLDrawable ) {
		...
		//gl.glLoadIdentity();
	    	...
	    	user.updateposition(0.05f, keys[KeyEvent.VK_W], keys[KeyEvent.VK_S], keys[KeyEvent.VK_A], keys[KeyEvent.VK_D], track);
	    	car1.updateposition(0.05f, keys[KeyEvent.VK_W], keys[KeyEvent.VK_S], keys[KeyEvent.VK_A], keys[KeyEvent.VK_D], track);
	    	car2.updateposition(0.05f, keys[KeyEvent.VK_W], keys[KeyEvent.VK_S], keys[KeyEvent.VK_A], keys[KeyEvent.VK_D], track);
	    	car3.updateposition(0.05f, keys[KeyEvent.VK_W], keys[KeyEvent.VK_S], keys[KeyEvent.VK_A], keys[KeyEvent.VK_D], track);

	    	user.checkcollisions(car_size, track, car1, car2, car3);

	    	if( !camera_switch )
	    	{
	    		glu.gluLookAt(user.carpx, user.carpy, user.carpz,
	    				user.carpx + (float)Math.cos((user.carangle) * ((float)Math.PI / 180.0f)),
	    				user.carpy + (float)Math.sin((user.carangle) * ((float)Math.PI / 180.0f)),
	    				user.carpz,
	    				0.0f,0.0f,1.0f);
	    	}
	    	else
	    	{
	    		if( camera == 1 )
	    		{
	    			glu.gluLookAt(car1.carpx, car1.carpy, car1.carpz,
	    					car1.carpx + (float)Math.cos((car1.carangle) * ((float)Math.PI / 180.0f)),
	    					car1.carpy + (float)Math.sin((car1.carangle) * ((float)Math.PI / 180.0f)),
	    					car1.carpz,
	    					0.0f,0.0f,1.0f);
	    		}
	    		else if( camera == 2 )
	    		{
	    			glu.gluLookAt(car2.carpx, car2.carpy, car2.carpz,
	    					car2.carpx + (float)Math.cos((car2.carangle) * ((float)Math.PI / 180.0f)),
	    					car2.carpy + (float)Math.sin((car2.carangle) * ((float)Math.PI / 180.0f)),
	    					car2.carpz,
	    					0.0f,0.0f,1.0f);
	    		}
	    		else if( camera == 3 )
	    		{
	    			glu.gluLookAt(car3.carpx, car3.carpy, car3.carpz,
	    					car3.carpx + (float)Math.cos((car3.carangle) * ((float)Math.PI / 180.0f)),
	    					car3.carpy + (float)Math.sin((car3.carangle) * ((float)Math.PI / 180.0f)),
	    					car3.carpz,
	    					0.0f,0.0f,1.0f);
	    		}
	    	}
		   ...
		   if( viewing_mode == FREE_VIEW_MODE )
		   {
		   ...
		   else if(viewing_mode == VEHICLE_MODE)
		    {
		    	...
		    	else
		    	{
		    		if( camera == 1 )
		    		{                      
		    			gl.glTranslatef( car1.carpx, car1.carpy, car1.carpz );
		    		}
		    		else if( camera == 2 )
		    		{
		    			gl.glTranslatef( car2.carpx, car2.carpy, car2.carpz );
		    		}
		    		else if( camera == 3 )
		    		{
		    			gl.glTranslatef( car3.carpx, car3.carpy, car3.carpz );
		    		}
		    	}
		    }
		...
		boolean color = true;
		gl.glDisable(GL2.GL_LIGHTING);
		for(float i = -30.0f; i < 30.0f; i += 7.50f)
		{
			for(float j = 0.0f; j < 10.0f; j+=1.0f)
			{
				...
				gl.glVertex3f(i, j*26.0f, 0.0f);
				gl.glVertex3f(i+7.50f, j*26.0f, 0.0f);
				gl.glVertex3f(i+7.50f, j*26.0f +26.0f,  0.0f);
				gl.glVertex3f(i, j*26.0f+26.0f,  0.0f);
				gl.glEnd();
				color=!color;
		...
		boolean color1 = true;
		gl.glDisable(GL2.GL_LIGHTING);
		for(float i = 0.0f; i < 30.0f; i += 7.50f)
		{
			for(float j = 0.0f; j < 10.0f; j+=1.0f)
			{
				if(color1)
		    	...
		    	gl.glBegin(GL2.GL_QUADS);
				gl.glVertex3f(0.0f, j*26.0f, i+7.50f);
				gl.glVertex3f(0.0f, j*26.0f, i);
				gl.glVertex3f(0.0f, j*26.0f +26.0f,  i);
				gl.glVertex3f(0.0f, j*26.0f+26.0f, i+7.50f);
				gl.glEnd();
				color1=!color1;
		...
		gl.glVertex3f(0.0f, 260.0f, 0.0f);
		gl.glVertex3f(0.0f, 260.0f, 30.0f);
		gl.glEnd();
		gl.glDisable(GL.GL_TEXTURE_2D);*/
		...
		gl.glEnable(GL.GL_TEXTURE_2D);
		gl.glEnable(GL2.GL_LIGHTING);
		OurCar.drawCarModel(gl, WRot += 10, 8.50f, 15.0f);
		gl.glDisable(GL.GL_TEXTURE_2D);
		gl.glDisable(GL2.GL_LIGHTING);
		gl.glPopMatrix();
		gl.glPushMatrix();
		...
		OurCat.drawCatModel(gl, CatRot = 180, 23.0f, 8.0f);
		gl.glPopMatrix();
		gl.glDisable(GL2.GL_LIGHTING);
		gl.glDisable(GL.GL_TEXTURE_2D);
		gl.glPopMatrix();
        ...
        TextRenderer renderer = new TextRenderer(new Font("Serif", Font.PLAIN, 25), true, true);
